...
function piChart_LinkFn($scope, element) {
  ...
  var numElems = $scope.sample.prediction
    ? demo.NUM_ELEMS_PREDICT
    : demo.NUM_NORMAL_ELEMS;
  ...
  function createChartConfiguration() {
    ...
    chartType = chartType || ['bar', 'line'][helpers.rnd(2)];
  ...
  function shuffle() {
    ...
    chartConfig.data.datasets.forEach(function (ds) {
      ds.data = helpers.generateRandomData(
        numElems,
        dsType || opType,
        s.prediction
      );
    ...
    for (var i = 0; i < numDatasets; i++) {
      ...
      ds.push({
        ...
        backgroundColor: !isBarChart
          ? helpers.color(color, 0.5)
          : helpers.generateColors(numElems, color, 0.5),
        showLine: isBarChart,
